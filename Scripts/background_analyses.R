# Background analyses
# In support of experimental design
# packages
library(tidyverse)
library(vegan)
library(patchwork)
...
herbivores_sum<-herbivores_complete %>%
  ...
  dplyr::summarise(fish_abund = sum(Count)) %>% # Sum herbivores
...
herb_plot_summary<-herb_plot_df %>%
  ...
  dplyr::summarize(mean = mean(fish_unit_area,na.rm = T),
            se = se_fn(fish_unit_area)) %>%
...
MRA_fish_plot<-ggplot(herb_plot_summary,aes(x=Habitat_long, y = mean)) +
...
ggsave(MRA_background_plot,
       filename = "./Figures//MRA_background.jpeg",
       dpi = 300,
       width = 5,
       height = 5)
